...
/* Esta funcion se utiliza para almacenar en el arreglo bidimensional las diferentes transacciones que representan las ventas de los diferentes productos. El fin datos esta dado por -1. */
{
    int MES, PRO, CAN;
...
/* Esta funciion se utiliza para leer los precios de venta de los diferentes productos. */
{
    int I;
    for (I=0; I<N; I++)
    {
...
/* Esta funcion se utiliza para calcular el monto de venta anual de cada uno de los productos. Observa que el resultado se almacena en un arreglo unidimensional que se utilizara posteriormente. */
{
    ...
    for (I=0; I< F; I++)
    {
        ...
        for (J=0; J<C; J++)
            SUM += A[I][J];
        V2[I] = V1[I] * SUM;
        printf("\nTotal de ventas del producto %d: %802f", I+1, V2[I]);
    }
}
void Funcion12(float A[], int C)
/* Esta funcion se utiliza para calcular el monto total de ventas de la fabrica. */
{
    ...
    printf("\n\nTiipo de Producto mas vendido: %d \t Ventas: %.2f", TPR + 1, VEN);
}
