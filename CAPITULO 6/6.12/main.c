...
void main(void)
{
    float LLU[PRO][MES][AÑOS];
    Lectura(LLU, PRO, MES, AÑO);
    Funcioin1(LLU, PRO, MES, AÑO);
    Funcioin2(LLU, PRO, MES, AÑO);
    Funcioin3(LLU, 18, MES, 5);
}
void Lectur (float A[][MES][AÑO],int F, int C, int P)
/* Esta funcion se utoliza para leer un arreglo tridimensional de tipo real de F filas, C columnas y P planos de profundidad. */
{
    int K, I, J;
    for (K=0; K<P; K++)
        for (I=0; I<F; I++)
...
/* Esta funcion se utilza para localizar el mes con mayor registro de lluvias en la provincia 18 en el quinto año. Escribe ademas el registro correspondiente. */
{
    int J, EMES = -1;
    float ELLU = -1.0;
    for (J=0; J<C; J++)
