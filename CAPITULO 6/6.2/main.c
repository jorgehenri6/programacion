...
/* Suma matrices.
El progrma, al recibir como datos dos arreglos bidimencionales del mismo tamaño, calcula la suma de ambos y la almacena en un tercer arreglo bodomensional. */
...
void main(void)
{
    ...
    do
    {
      printf("Ingrese el numero de columnas de losarreglos: ");
      scanf("%d",&COL);
    }
    while (COL > MAX || COL < 1);
    /* Se verifica que el columnas sea correcto. */
    printf("\nLectura del Arreglo MA\n");
    Lectura(MA, FIL, COL);
    printf("\nLectura del Arreglo MB\n");
    Lectura(MB, FIL, COL);
    Suma (MA, MB, MC, FIL, COL);
    printf("nImprecion del Arreglo MC\n");
    Imprime (MC, FIL, COL);
}
