...
/* Alumnos.
El programa, al recibir un arreglo bidimensional que contiene informancion sobre calificaciones de alumnos en cuatro materias diferentes, obtiene resultados estadisticos. */
...
void main(void)
{
    ...
    /* Esta funcion se utiliza para obtener el promedio de cada estudiante. */
    {
        ...
        for (I=0; I<T; I++)
        {
            ...
            for (J=0; J<EXA; J++)
                SUM += A[I][J];
            PRO = SUM / EXA;
            printf("\nEl promedio del alumno %d es: %5.2%", I+1, PRO);
        }
    }
    ...
    /* Esta funcion se utiliza tanto para obtener el promedio de cada examen, asi como tambien el examen que obtuvo el promedio mas alto. */
    {
        ...
        printf("\nEl promedio del examen %d es: %d es:", J+1, PRO);
    }
